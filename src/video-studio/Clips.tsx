...
type Job={id:string;sceneId:string;name:string;state:string;estimatedUsd:number;url:string|null;expiresAt:string};
...
export default function Clips({scene,blocked}:{scene:Scene;blocked:boolean}){
 ...
 async function run(action:'quote'|'start'|'refresh'){
  ...
  try{
   if(action==='quote'){const s=signature;const d=await request({action,scene});if(latest.current===s)setQuote({job:d.job,basis:s});}
 ...
 {jobs.length>0&&<div className="vs-clips"><h4>Tus últimas tomas</h4>{jobs.map(j=><article key={j.id}><strong>{j.name}</strong><p>{labels[j.state]||j.state}</p>{j.url?<><video src={j.url} controls preload="metadata" playsInline/><a href={j.url} target="_blank" rel="noreferrer">Abrir / descargar clip</a></>:null}<small>Costo estimado: USD {j.estimatedUsd.toFixed(3)}</small></article>)}</div>}

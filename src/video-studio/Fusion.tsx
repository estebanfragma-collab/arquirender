...
export default function Fusion({scene,assets,panel}:{scene:Scene;assets:RenderAsset[];panel:HTMLElement|null}){
 ...
 async function generate(){
  ...
  try{
   ...
   setResult({url:url.current,name:'Fusión hacia vista general'});setMessage('Fusión lista. Descárgala para conservarla o añadirla en Edición.');
 ...
 return <section className="vs-generation"><h3>Fusión controlada</h3><p>La primera imagen se desvanece mientras aparece la vista general durante 1,5 segundos. Se conservan las dos imágenes, sin movimiento de dron ni cambios generados por IA.</p><small>MP4 · sin audio · sin costo de generación. El video se crea en tu dispositivo. Descárgalo antes de cerrar.</small>
 <div className="vs-generate-dock"><div className="vs-take-summary"><strong>Fusión hacia vista general</strong><small>2 imágenes · {scene.duration} segundos · {scene.format} · sin costo</small></div><button className="vs-primary" disabled={busy||!start||!end||start.id===end.id} onClick={generate}>{busy?'Creando fusión…':'Crear fusión suave · sin costo'}</button>{(!start||!end)&&<small>Selecciona la imagen inicial y la vista general final.</small>}<p role="status">{message}</p></div>
 {panel&&result&&createPortal(<article className="vs-local-fusion"><h3>{result.name}</h3><p>Fundido controlado · creado en este dispositivo</p><video controls src={result.url} playsInline/><a href={result.url} download="fusion-vista-general.mp4">Descargar fusión MP4</a><small>Descarga el archivo y añádelo en Edición. No se guarda en el historial de generaciones.</small></article>,panel)}
 </section>;
}

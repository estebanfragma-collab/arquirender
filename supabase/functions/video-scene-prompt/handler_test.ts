...
const input={images:['data:image/jpeg;base64,/9j/AAAA'],brief:'Luz natural',mode:'animate',movement:'push',duration:5,format:'16:9'};
...
function mock(opts:{auth?:boolean;permit?:string;provider?:number;refusal?:boolean}={}){
  ...
  const fetcher:typeof fetch=async(url,init)=>{
    ...
    return Response.json({model:body.model,choices:[{finish_reason:'stop',message:opts.refusal?{refusal:'no'}:{content:JSON.stringify({prompt:'Slow dolly-in toward the facade.',notes:'Acercamiento suave.'})}}]},{status:opts.provider||200});
  };
...
Deno.test('real image input is passed and structured proposal returned',async()=>{const m=mock();const r=await m.handler(req());assert(r.status===200);assert((await r.json()).prompt==='Slow dolly-in toward the facade.');assert(m.calls.length===3);});
...
Deno.test('transition requires exactly two frames and known settings',async()=>{const m=mock();for(const data of [{...input,mode:'transition'},{...input,movement:'orbit'},{...input,duration:15},{...input,format:'bad'}])assert((await m.handler(req(data))).status===400);assert(m.calls.every(c=>c.endsWith('/user')));const r=await m.handler(req({...input,mode:'transition',images:[...input.images,...input.images]}));assert(r.status===200);});
